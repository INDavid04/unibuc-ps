...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Setări generale
      sliderInput("n_sim", "Număr de cereri simulate:", min = 100, max = 10000, value = 1000, step = 100),
      ...
      sliderInput("media_exp_ms", "Media (ms):", min = 50, max = 1000, value = 200, step = 10),
      ...
      sliderInput("mu_norm_ms", "Media (mu - ms):", min = 50, max = 1000, value = 200, step = 10),
      sliderInput("sigma_norm_ms", "Deviația Standard (sigma - ms):", min = 10, max = 300, value = 50, step = 5)
    ),
    
    mainPanel(
      tabsetPanel(
        tabPanel("Distribuții (Histograme)",
...
server <- function(input, output) {
  ...
  output$plotExponential <- renderPlot({
    ...
    ggplot(df, aes(x = x)) +
      geom_histogram(aes(y = ..density..), binwidth = 10, fill = "orange", color = "white", alpha = 0.7) +
      stat_function(fun = dexp, args = list(rate = lambda), color = "red", lwd = 1.2) +
      labs(title = paste("Distribuția Exponențială (Media =", media, "ms, Lambda =", round(lambda, 5), ")"),
  ...
  output$plotNormal <- renderPlot({
    ...
    ggplot(df, aes(x = x)) +
      geom_histogram(aes(y = ..density..), binwidth = 10, fill = "purple", color = "white", alpha = 0.7) +
      stat_function(fun = dnorm, args = list(mean = mu, sd = sigma), color = "blue", lwd = 1.2) +
      labs(title = paste("Distribuția Normală (Mu =", mu, "ms, Sigma =", sigma, "ms)"),
           x = "Timp de răspuns (milisecunde)", y = "Densitate") +
  ...
  output$tabelStatistici <- renderTable({
    ...
    data.frame(
      Metrica = c("Media (Mean)", "Mediana (Median)", "Valoarea Modală (Mode)", "Varianța (Variance)"),
  ...
  output$textDiscutie <- renderUI({
    ...
    HTML(paste0(
      ...
      "<p>",
      "Observați că <b>Media > Mediana</b> (diferență aprox: ", round(dif_exp, 1), " ms). <br>",
      "Acest lucru indică o asimetrie la dreapta. Câteva cereri lente ('coada lungă') cresc media semnificativ, ",
      "în timp ce mediana rămâne mai stabilă și mai reprezentativă pentru utilizatorul 'tipic'.</p>"
    ))
  })
}
